...
Prime.Dom.Element.prototype = {
  ...
  addEventListener: function(event, handler, context) {
    ...
    if (event.indexOf(':') == -1) {
      ...
      //custom event
      this.customEvents[event] = this.customEvents[event] || [];
      this.customEvents[event].push(proxy);
    }
  ...
  fireEvent: function(event, memo, bubbling, cancellable) {
    ...
    if (event.indexOf(':') == -1) {
      ...
      for (index in this.customEvents[event]) {
        if (this.customEvents[event].hasOwnProperty(index)) {
          this.customEvents[event][index](evt);
        }
      }
    }
  ...
  removeEventListener: function(event, handler) {
    if (event.indexOf(':') == -1) {
      ...
      if (this.domElement.removeEventListener) {
        this.domElement.removeEventListener(event, handler, false);
      } else if (this.domElement.detachEvent) {
        this.domElement.detachEvent('on' + event, handler);
      } else {
      ...
      this.customEvents[event] = this.customEvents[event] || [];
      var newEvents = [];
      for (var ii = 0; ii < this.customEvents[event].length; ii++) {
        var proxy = this.customEvents[event][ii];
        if(proxy !== handler) {
          newEvents.push(proxy);
        }
      }
      this.customEvents[event] = newEvents;
    }
